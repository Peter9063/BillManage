...
public class ExcelReadUtils4BigFile {
	
	private static Logger logger = LoggerFactory.getLogger(ExcelReadUtils4BigFile.class);
	 ...
	 */
	public static void createSheetForExcel(SXSSFWorkbook work,List<List<String>> sheetList,String title,int i){
		SXSSFSheet sheet=work.createSheet(title+"_"+i);
		sheet.setDefaultColumnWidth(15);
		//设置内容样式
		CellStyle numStyle = work.createCellStyle(); 
		...
		//设置标题样式
		CellStyle numStyleHead = work.createCellStyle();
		Font fontHead  = work.createFont();
		fontHead.setFontHeightInPoints((short) 25);// 字号   
		fontHead.setBold(true);
		numStyleHead.setFont(fontHead);
		numStyleHead.setAlignment(HorizontalAlignment.CENTER);
		numStyleHead.setVerticalAlignment(VerticalAlignment.CENTER);
		...
		Cell cellHead = headerRow.createCell(rowCounter);
...
//					sheet.groupRow(startNum+1, i-1);
	...
	public static byte[] createExcel(String title,List<String> subTitle,List<List<String>> datas) throws IOException {
		...
		//设置内容样式
		HSSFFont font  = ExcelReadUtils4BigFile.getContentFontStyle(work); 
		HSSFCellStyle numStyle = ExcelReadUtils4BigFile.getContentCellStyle(work,font);
		
		//设置标题样式
		HSSFFont fontHead  = ExcelReadUtils4BigFile.getHeadFontStyle(work);
		HSSFCellStyle numStyleHead = ExcelReadUtils4BigFile.getHeadCellStyle(work,fontHead); 
